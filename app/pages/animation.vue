...
const selectedYear = useState('vfq_animation_year', () => '')

const displayLimit = useState('catalog_display_limit_animation', () => 40)
...
const availableYears = computed(() => {
  if (!allAnimation.value) return []
  const years = allAnimation.value
    .map(m => extractYear(m))
    .filter(y => y !== null)
...
const filtered = computed(() => {
  if (!allAnimation.value) return []
  let list = allAnimation.value.filter(m => {
...
useHead({
  title: 'Animation & Jeunesse VFQ - Doublage Québec'
})
</script>
